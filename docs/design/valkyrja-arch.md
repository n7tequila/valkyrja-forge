...
> 起草依据来自试点项目首次端到端运行中 `demo-form-flow` 的真实 design.md 产出。
> 实施顺序按 D9：skill 本体 + 最小 catalog → 试点项目 架构工作区 → 回填 design.md → 恢复测试主线。
>
> **脱敏声明（D6）**：本文档涉及试点项目的一切名称——change 名、DOMAIN、
> 契约名、业务词汇——均为脱敏代号，非实盘记录值；「铸造即冻结」等条款
> 冻结的是试点仓内的实值，本文档的代号不承载该约束。
...
- **D5 第一波 catalog 清单（已批准，约 20 条）**：经评审 内部项目规范体系
  ...
  - 内部项目派生条目入公开仓前**必须去业务化**（剥离领域实体与业务错误码等细节，
...
- **D6 许可证纪律（四条 + 一边界）**：
  ...
  （修订 2026-08-22：第 1 项已**机检化**——客户缩写第二次同型漏网（设计文档三处、
  已推送）后，人工检查单证明不可依赖。`scripts/check-sanitization.sh` 对 git
  跟踪全量文件扫描私有词表（`~/.claude/valkyrja/d6-wordlist.txt`，词表本身即
  敏感内容故不入库；词表含主干词防前缀替换类残留），命中即退出码 1，
  建议接 pre-push/CI；词表缺失显式报「跳过」不静默。）
...
- **D9 接回 试点项目 测试的顺序 = arch 先行、最小可用**：
  ...
  (3) 回填 demo-form-flow 的 design.md，三个空缺段改为真实 `依据: ADEC-*`
  引用——同时是 V4.8 首次实战；
...
- **D12 多项目共享模型（2026-08-21，apply 前讨论裁决）**：
  ...
  **契约三分类**（本项当日生效，c 类为试点项目下一步铸契约的真实形态）：
  ...
  **留待组织仓真实出现再裁的问题清单**（不预铸规则）：
  ...
  其余机制条款随实现一并入。试点仓的 apply 不被本项阻塞。
...
| 空缺 | design.md 中的表现 |
|---|---|
| 幂等标识约定 | 表单补传与埋点上报都要幂等，但标识如何生成、什么格式、两者是否共用一套规则，本 change 只能自己定 |
| 错误与提示约定 | 超限 / 格式不合法 / 网络失败三类情况，文案层级、可重试性、是否上报——无项目级规范可依 |
| **跨 change 共享接口契约** | 同意文案版本号随内容包下发，这是本 change 与 `demo-pkg-sync` 的共享约定。它既不属于 spec（不是可观察的产品行为），又不该只写在某一份 design.md 里（另一个 change 看不到） |
