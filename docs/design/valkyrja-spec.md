...
| **背书铸造时** | 背书 DEC 必然 round > release 却无可发内容——V2.5 会把每次视觉背书都当欠账拦住 propose，无限死锁 | DEC 可选标记 `frid-impact: none`（不改 FRID 语义的背书类/流程类决策）豁免欠账；信任边界=标记须在 decide 确认回显显式出示。首个实例：DEC-023 视觉基线背书 |
| **首单 propose 前** | 交接单预存粘贴段 = 派生值落盘：承袭后路径过期（v1.2 交接单仍指 v1.1，实拦一次）、提示永远缺 decompose 后才出生的权威（DEC-023 不在任何预存段里）；用户直问"这么多内容谁来写" | 交接单瘦身只存裁决字段；粘贴段改为**现算生成**（decompose 只读伴生）——人手写量归零，与 config 注入构成刻意双层冗余，trace 机检终审 |
| **首单 propose 后** | 用户问"怎么主动触发 trace"并提案：干脆把 propose/apply/archive 全包进自己的壳，门禁过了再调官方——传统代码的 facade 模式 | **三动词壳**（用户设计）：归档本就是壳（trace→确认→代跑 CLI），补齐 propose/apply 同款——门禁在前、确认在中、委托在后、产后自动检查；"怎么触发 trace"问题消解（内建于动词）。诚实边界保留：/opsx 侧门存在，约定级收口非强制。另修 trace.py V4.0 正则（Authority 块在文件末尾被误报缺块——首个真实 propose 即此形态，假阳性 ERROR） |
...
| **第二轮 propose 后（用户读回显反馈）** | 确认回显代号密度过高：FRID/DEC/V 号/D 号裸奔，用户逐个查证成本过高，「结论」埋在集合运算之后 | 三技能确认节统一新增**回显可读性（代号不裸奔）**规则（三份逐字一致，漂移可 grep）：代号首次出现内联一句释义 + 权威文件相对路径（终端可点击）；释义现读现算不凭记忆（说错比裸奔更危险）；先归纳后证据（顶部总账 → 每条一句人话 → 细节进括号）；追问展开原文。只约束呈现，不放宽「回显必须包含」的完整性 |
| **消费仓无常驻护栏（用户提问「要不要 CLAUDE.md」）** | 全部护栏都依赖技能被触发；技能不触发时护栏为零——有人直接手改 `prd/releases/**`（协议明令不可变）或主 spec，没有任何机制在场，机检也只在跑 trace 时才发现后果。触发面收紧为工作区锚定 opt-in 后此缺口更大。实测：首个消费仓根目录既无 CLAUDE.md 也无 AGENTS.md，压力全被挤进 `openspec/config.yaml`（它只在官方 skill 的 prompt 内生效），并已漂进 initiative 专属路径——违反其自身头部禁令 | **消费仓 CLAUDE.md 治理块**：`templates/claude-guard-block.md`（三技能各一份、逐字一致、md5 可验，因技能可独立安装）+ 三处 SKILL.md 同款规则。设计要点：块内**只写归属与禁令、不复述任何协议规则、不写派生值**（否则又是一个会漂移的载体）；界定块 `valkyrja:begin/end` 幂等 upsert，已有文件只插块、不动其他内容，文件不存在只创建含块的文件（项目概述交给 `/init`，两边各写必打架）；写入是特权动作须回显确认；诚实标注 prompt 级非强制。**时机**＝首次落盘治理文件后 + 任何特权确认时发现块缺失（后者覆盖被外部工具抹掉与协议升级前的老项目——它们永远不会再经过任何「首次」）。配套：arch 的 bootstrap 探测既有规范时**跳过该块**，否则会把自己的产物当独立既有事实读回来（空白项目无其他事实稀释，最易踩） |
...
**拆分回显保底**——baseline 确认只出示提取物、不对照 PRD 原文，
...
| 文件 | 内容 | 何时加载 |
|---|---|---|
| `SKILL.md`（540 行） | 宪法、术语与集合、启动仪式、三契约摘要、意图路由、确认、各动作流程、对外接口 | 每次调用 |
