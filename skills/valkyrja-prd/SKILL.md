---
name: valkyrja-prd
description: 产品需求工作坊——支持跨会话的需求讨论、决策沉淀、外部文档导入与 PRD 合成发布。当用户想讨论产品需求、整理需求思路、记录或确认产品决策、导入需求文档/会议纪要/客户材料、把一批存量历史文档批量初始化进需求工作区、收编或评审系统原型（Figma导出/生成HTML/图稿）并背书为视觉基线、询问需求进展状态、生成或更新或发布 PRD 时，必须使用本技能。即使用户只是随口说"我们聊聊 XX 功能"、"这个就这么定了"、"看看还有什么没讨论的"、"整理一版需求文档"、"评审一下原型"、"背书为视觉基线"，只要项目已有 docs/product/initiatives/ 工作区，或用户明确要在本项目建立需求治理工作区，都应触发本技能。未 opt-in 的项目（无该工作区且用户未要求建立）不要接管，也不要主动提议初始化。
---
...
**回显可读性（代号不裸奔）**：面向用户的确认回显与判断清单中，任何 ID/代号
（FRID、DEC/ADEC、Q、V 检查号等）首次出现必须内联一句释义，并附其权威文件的
仓库相对路径（终端可点击打开）。释义**现读现算**——取自该文件的标题或首句，
不得凭记忆复述：回显把内容说错比裸代号更危险。结构**先归纳后证据**：顶部
一行总账（这批共几件、各是什么性质），每条先用一句人话讲清「这是什么事、
为什么需要你过目」，代号与集合运算细节收进括号或次行；用户追问任一条时
...
本动作只产出待注入内容与背书 DEC id，不越层代写；交接走 spec 的
**「背书类权威通道」**（准入 = `frid-impact: none` 标记，不得携带需求语义）。
...
**发版欠账**（现算：`round` 大于最近 release round 的 DEC，
**剔除带 `frid-impact: none` 标记者**——背书类无可发内容，与 V2.5 门限同判据，
仪表盘与门禁不得给出相反结论。
...
- [prototype-manifest.md](templates/prototype-manifest.md) — 原型基线包清单
