...
description: 重构审查，只审不改。扫描指定范围的代码与 git 历史，按「局部重构、结构抽象、设计模式」三级阶梯判断哪里值得重构、哪里不该动，每条给出 REFACTOR / KEEP / REJECT / TESTS_FIRST / REWRITE 之一，并按级别分层确认。当用户说「看看哪里值得重构」「这几个模块做完了，该重构了吗」「这个 switch 要不要改成 Strategy」「要不要抽接口 / 抽象类」「是不是过度设计了」「评估一下这段代码的结构」「对一下台账」「上次审的做到哪了」时使用。审完留一份重构台账，下次审查先对账；没有值得动的也留观察哨与下次建议。不执行重构、不清死代码、不简化刚改完的 diff，那些交给别的工具。
---
...
一样是它的对象。它还留台账：这次裁了什么、什么条件下回头看，下次审查先对账再开工。

## 硬边界

- **不改代码，不提交。** 产出是对话里的报告与确认后的执行清单，外加一份重构台账——唯一落盘物，
  写前回显、等确认，见 [references/ledger.md](references/ledger.md)。
...
找台账，有就先对账（见 ledger.md）：上次的待做对提交号，观察哨核触发条件，触发的进本次候选。
...
3. **收尾**：汇总一屏就是执行清单；紧接着提议写台账——有就追加，没有按默认路径新建，回显后
   一个问题：写入 / 换路径 / 这次不写。没有值得动的也收尾，台账里进观察哨与下次建议。
...
KEEP / REJECT 各带一条触发（可核对的条件 → 动作）或标「不动」；进台账成观察哨。
...
唯一的特权动作是写台账：先回显路径与内容，等用户明确「确认」再写；疑问语气视为倾向，不写。
...
| [references/ledger.md](references/ledger.md) | 台账格式、默认路径、状态、观察哨、对账、收尾、空结论 | 定范围时对账；收尾时写 |
